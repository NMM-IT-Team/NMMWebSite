    ...
    public class EventDetailsViewModel : BaseViewModel
    {
        ...
        private void OpenDirectionInMaps()
        {
            ...
            switch (Device.RuntimePlatform)
            {
                case Device.iOS:
                    Device.OpenUri(
                        new Uri(string.Format("http://maps.apple.com/?q={0}", WebUtility.UrlEncode(SelectedEvent.EventAddress))));
                    break;
                case Device.Android:
                    Device.OpenUri(
                        new Uri(string.Format("geo:0,0?q={0}", WebUtility.UrlEncode(SelectedEvent.EventAddress))));
                    break;
            }
        }
